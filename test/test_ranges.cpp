...
BOOST_AUTO_TEST_CASE(numeric_row_range_test)
{
    ...
    grid[0].push_back("1"); grid[1].push_back("4");
    grid[0].push_back("2"); grid[1].push_back("5");
    grid[0].push_back("3"); grid[1].push_back("6");

    row_range range(in);

    std::size_t i = 0;
    for (row_range::iterator r = range.begin(), e = range.end();
         r != e; ++r) {
...
BOOST_AUTO_TEST_CASE(map_row_range_test)
{
    ...
    std::size_t i = 0;
    for (map_row_range::iterator r = range.begin(), e = range.end();
         r != e; ++r) {
